...
\begin{frame}
  ...
  Исследуется задача диагностики ишемической болезни по электрокардиограмме высокого разрешения (2000Гц, в отличие от стандартных 100 Гц)

  \vspace{0.5cm}

  Исследование базируется на результатах, полученных проф. К.В. Зайченко и его научной школой (СПбГМУ, ГУАП) по моделированию развития ишемии миокарда на крысах.
...
\begin{frame}
  ...
  $$ \widehat{f}(\omega) = \int_{-\infty}^{\infty}\widetilde{ecg}(t) e^{-it\omega}dt $$
\end{frame}
...
\begin{frame}
  ...
  Существует множество различных методик детектирования R зубцов.
...
\begin{frame}
  ...
  $$ \widehat{f_i}(\omega) = \int_{-\infty}^{\infty}W_{\Delta \tau_i}(t)\widetilde{ecg}(t) e^{-it\omega}dt $$
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}

  Аналогично предыдущему построению выделяются признаки:
